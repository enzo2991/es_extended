...
MySQL.ready(function()
  ...
  if not Jobs2 then
    -- Fallback data, if no jobs exist
    ESX.Jobs2['unemployed'] = {label = 'Unemployed',
                              grades = {['0'] = {grade = 0, label = 'Unemployed', salary = 200, skin_male = {}, skin_female = {}}}}
  else
    ESX.Jobs2 = Jobs2
  end
  ...
  print('[INFO] ESX Legacy 1.8.5 initialized!')
  StartDBSync()
  StartPayCheck()
end)
...
RegisterServerEvent("esx:refreshjobs")
...
RegisterServerEvent("esx:refreshjobs2")
AddEventHandler("esx:refreshjobs2",function()
  local jobs2 = MySQL.query.await('SELECT * FROM jobs2')
  ...
  for k,v in ipairs(ESX.Jobs2) do
    if ESX.Table.SizeOf(v.grades) == 0 then
      ESX.Jobs2[v.name] = nil
      print(('[^3WARNING^7] Ignoring job ^5%s^0 due to no job grades found'):format(v.name))
    end
  end
end)
