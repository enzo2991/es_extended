function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords,storepoint)
 ...
 -- Store Point
	function self.setStorePoint(quantity, reason)
		reason = reason or 'unknown'
		if not tonumber(money) then 
			print(('[^1ERROR^7] Tried To Set Store point For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(self.playerId, quantity))
			return
		end
		if quantity > 0 then
			local Spoint = self.getStorePoint()

			if Spoint then
				quantity = Spoint.Round and ESX.Math.Round(Spoint) or Spoint
				self.storepoint = quantity

				self.triggerEvent('esx:setStorePoint', Spoint)
				TriggerEvent('esx:setStorePoint', self.source, quantity, reason)
			else 
				print(('[^1ERROR^7] Tried To Set Add To Invalid Store point For Player ^5%s^0!'):format(self.playerId))
			end
		else 
			print(('[^1ERROR^7] Tried To Set Account ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(self.playerId, quantity))
		end
	end
	
	function self.addStorePoint(quantity, reason)
		reason = reason or 'unknown'
		if not tonumber(money) then 
			print(('[^1ERROR^7] Tried To Add Store point For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(self.playerId, quantity))
			return
		end
		if quantity > 0 then
			local Spoint = self.getStorePoint()
			...
			if Spoint then
				...
				self.triggerEvent('esx:setStorePoint', Spoint)
				TriggerEvent('esx:addStorePoint', self.source, quantity, reason)
			else 
				print(('[^1ERROR^7] Tried To Add Store point To Invalid Account ^5%s^0 For Player ^5%s^0!'):format(self.playerId))
			end
		else 
			print(('[^1ERROR^7] Tried To Add Store point ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(self.playerId, quantity))
		end
	end

	function self.removeStorePoint(quantity, reason)
		reason = reason or 'unknown'
		if not tonumber(money) then 
			print(('[^1ERROR^7] Tried To Add Store point ^5%s^0 For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(self.playerId, quantity))
			return
		...
		if quantity > 0 then
			...
			if Spoint then
				...
				self.triggerEvent('esx:setStorePoint', Spoint)
				TriggerEvent('esx:removeStorePoint', self.source quantity, reason)
			else 
				print(('[^1ERROR^7] Tried To Set Add To Invalid store point For Player ^5%s^0!'):format(self.playerId))
			end
		else 
			print(('[^1ERROR^7] Tried To Set StorePoint For Player ^5%s^0 To An Invalid Number -> ^5%s^7'):format(self.playerId, quantity))
		end
	end
	...
	function self.setJob2(job, grade)
		...
		if ESX.DoesJobExist2(job, grade) then
			local jobObject, gradeObject = ESX.Jobs2[job], ESX.Jobs2[job].grades[grade]
