...
SetGameType('Outlawz V2')

local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `job2`, `job2_grade`, `group`, `position`, `inventory`, `skin`, `loadout`,`store_point`, `id`'
...
function loadESXPlayer(identifier, playerId, isNew)
  ...
  if result.store_point then 
    userData.storepoint = result.store_point
  end 
  ...
  local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.job2,
    userData.loadout, userData.playerName, userData.coords, userData.storepoint)
